...
public class ConcurrentDoublyLinkedList<E> extends AbstractCollection<E> implements java.io.Serializable {
	 ...
	 * first-to-last order. The <tt>next</tt> method returns elements reflecting
	 * the state of the deque at some point at or since the creation of the
	 * iterator. The method does <em>not</em> throw
...
class Node<E> extends AtomicReference<Node<E>> {
	 ...
	 */
	private static final long serialVersionUID = -2245356546557218432L;
	 ...
	 * point back to this. Returns null if this node is trailer so has no
	 * successor.
	 * 
	 ...
	 * Returns null if this node is header so has no successor. May also return
	 * null if this node is deleted, so doesn't have a distinct predecessor.
	 * 
	 ...
	 * Use for backward list traversal, but beware that if this method is called
	 * from a deleted node, it might not be able to determine a usable
	 * predecessor.
	 * 
	...
	/**
	 * Tries to insert a node holding element as successor, failing if this node
	 * is deleted.
	 * 
	 * @param element
	 *            the element
